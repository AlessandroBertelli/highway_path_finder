...
int main() { //rimuovi argv
    ...
    FILE *fp;
        ...
        //rimuovi
        fp = fopen("/Users/alessandrobertelli/Downloads/archivio_test_aperti/open_1.txt", "r");
        if ((fp) == NULL) {
            fprintf(fo, "Errore nell'apertura del file");
            exit(1);
        }

        fo = fopen("/Users/alessandrobertelli/Downloads/archivio_test_aperti/open_1_output.txt", "wt");
        if ((fo) == NULL) {
            fprintf(fo, "Errore nell'apertura del file");
            exit(1);
        }
